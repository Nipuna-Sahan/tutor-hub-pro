...
const AdminPayments = () => {
  ...
  const paidStudents = Math.max(0, students.length - pendingPayments);
  ...
  const stats = [
    { title: "Total Revenue", value: `Rs. ${totalRevenue.toLocaleString()}`, icon: DollarSign, color: "text-success", bgColor: "bg-success/10", change: "+12%" },
    { title: "Monthly Revenue", value: `Rs. ${monthlyRevenue.toLocaleString()}`, icon: TrendingUp, color: "text-primary", bgColor: "bg-primary/10", change: "+8%" },
    { title: "Paid Students", value: paidStudents, icon: Users, color: "text-info", bgColor: "bg-info/10", change: students.length > 0 ? `${Math.round((paidStudents / students.length) * 100)}%` : "0%" },
    { title: "Pending", value: pendingPayments, icon: Calendar, color: "text-warning", bgColor: "bg-warning/10", change: "Due" },
  ];
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-base md:text-lg">
              <CreditCard className="w-4 h-4 md:w-5 md:h-5 text-primary" />Recent Payments
            </CardTitle>
            ...
            {recentPayments.map((payment) => (
              <div key={payment.id} className="flex items-center justify-between p-3 rounded-xl bg-muted/30 hover:bg-muted/50 transition-colors">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <Badge variant={payment.status === "paid" ? "default" : "secondary"} className="text-xs capitalize">{payment.status}</Badge>
                </div>
              </div>
            ))}
          </div>
